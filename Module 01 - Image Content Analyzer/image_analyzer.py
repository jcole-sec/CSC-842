...
def parse_keywords(keyword_file):
    ...
    with open(keyword_file, 'r') as f:
        keywords = [str(x).lower().rstrip() for x in
                    f.readlines()]  # convert to lower case and remove trailing newline characters
    return keywords


def get_images(srcdir):
    """ Recurse through directory path to identify images files using magic-byte enumeration """
    image_files = []
